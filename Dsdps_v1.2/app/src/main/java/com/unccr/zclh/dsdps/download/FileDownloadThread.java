...
class FileDownloadThread extends Thread {
    ...
    public void run() {
        ...
        Log.d(TAG, "run downloadSuccess: " + downloadSuccess);
        try {
            Thread.sleep(500);
    ...
    private boolean initCurrentProgress() {
        ...
        Log.d(TAG, "initCurrentProgress currentProgress: " + currentProgress + " ,fileName: " + fileName + " ,threadNum: " + threadNum);
        return false;
    }
    ...
    private boolean brakePointDownload() {
        while (retryNum < retrySize && !cancelTask) {
            try {
                initDownloadLog();
                Log.d(TAG, "brakePointDownload brakePointDownload retryNum: " + retryNum + " ,downloadFileUrl: " + downloadFileUrl);
//				URL httpUrl = new URL(downloadFileUrl);
                ...
                Log.d(TAG, "brakePointDownload brakePointDownload responseCode:" + httpURLConnection.getResponseCode() + ", newValue : " + newValue);
                httpURLConnection.connect();
                if (HttpURLConnection.HTTP_OK == httpURLConnection.getResponseCode() ||
                        HttpURLConnection.HTTP_PARTIAL == httpURLConnection.getResponseCode()) {
                    Log.d(TAG, "brakePointDownload brakePointDownload newValue:" + newValue + ",contentLength:" + httpURLConnection.getContentLength()
...
//		int downloadProgress = SQLiteManager.getInstance().queryProgresToTable(fileName, threadNum);
        if (NOTRECORD == downloadProgress) {
            Log.d(TAG, "initDownloadLog initDownloadLog downloadProgress record is zero");
            ...
            Log.d(TAG, "initDownloadLog initDownloadLog startPosition: " + startPosition + " ,currentProgress: " + currentProgress + " ,threadNum: " + threadNum + " ,fileName: " + fileName);
        }
//		if (startPosition >  0 && startPosition == currentProgress) {//如果当前下载的进度和当前的进度相同
//			Log.d(TAG, "initDownloadLog true");
//			return true;
//		}
        Log.d(TAG, "initDownloadLog initDownloadLog threadid: " + threadNum + " ,downloadProgress: " + downloadProgress + " ,fileName: " + fileName);
        return false;
    }
    ...
    private void randomWriterFile(InputStream httpInputStream, int brakePoint) throws IOException {
        ...
        while ((length = bufferedInputStream.read(buffer)) != downloadEnd) {
            ...
            Log.d(TAG, "randomWriterFile randomWriterFile currentProgress: " + currentProgress + " ,threadId: " + threadNum);
        ...
        //显示下载完成
        Log.d(TAG, "randomWriterFile threadId: " + threadNum + " ,download over" + " ,download size:" + currentProgress + ",per downloadSize:" + (fileEndPosition - startPosition));
    }
    ...
    public void cancelTask() {
        Log.d(TAG, "cancelTask threadId: " + threadNum + " cancelTask");
        retryNum = retrySize;
        cancelTask = true;
        if (null != httpURLConnection) {
            Log.d(TAG, "cancelTask httpURLConnection not null threadId: " + threadNum + " cancelTask");
            httpURLConnection.disconnect();
        }
        SQLiteManager.getInstance().deleteTable(fileName, threadNum);
    }

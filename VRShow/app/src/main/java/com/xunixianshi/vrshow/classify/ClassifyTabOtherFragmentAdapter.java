 ...
 */
public class ClassifyTabOtherFragmentAdapter extends BaseAda<MainOtherTabListObj> {
    ...
    public ClassifyTabOtherFragmentAdapter(Context context) {
        super(context);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final MainOtherTabListObj item = getItem(position);
        ...
        PicassoUtil.loadImage(mContext, item.getCoverImgUrl() + "?imageView2/2/w/" + (screenWith + 120), viewHolder.public_video_item_icon_iv);
        ...
        viewHolder.public_video_item_bottom_text_two_tv.setText("播放次数：" + item.getPlayerTotal());
        ...
        attachZanView(viewHolder,item.getIsLike(),item.getLikesTotal());
        ...
        viewHolder.public_video_item_download_ll.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                shareInterface.downLoad(item.getResourceId(), item.getCoverImgUrl(), item.getResourceTitle());
            }
        });
        viewHolder.public_video_item_share_ll.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                shareInterface.share(item.getResourceSmallIntro(), item.getCoverImgUrl(), item.getResourceTitle(), item.getResourceId());
            }
        });
     ...
     */
    private void zanVideo(final ViewHolder viewHolder, final MainOtherTabListObj item) {
        // 加密用户ID
        String uid;
        if (AppContent.UID.equals("")) {
            uid = "-1";
...
//                        showToastMsg("点赞成功");
                    ...
                    attachZanView(viewHolder,result.getLikesStatus(),result.getLikesTotal());
                } else {
//                    showToastMsg(result.getResDesc());
     ...
     * 设定文件 @return void 返回类型 @throws
     */
    private void collectVideo(final ViewHolder viewHolder, final MainOtherTabListObj item) {
        // 加密用户ID
        ...
        OkHttpAPI.postHttpData("/user/resources/collection/service", jsonData, new GenericsCallback<ResourcesCollect>() {
            ...
            public void onResponse(ResourcesCollect result, int id) {
                if (result.getResCode().equals("0")) {
                    ...
                    item.setCollectId(result.getCollectId());
                } else {
...
//                showToastMsg("网络连接失败!请检查网络");
    ...
    private void cancelCollectVideo(final ViewHolder viewHolder, final MainOtherTabListObj item) {
        // 加密用户ID
        String Ras_uid = VR_RsaUtils.encodeByPublicKey(AppContent.UID);
        // 封装参数
        HashMap<String, Object> hashMap = new HashMap<String, Object>();

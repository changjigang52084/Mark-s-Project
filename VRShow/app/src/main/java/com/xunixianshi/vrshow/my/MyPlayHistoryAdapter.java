 ...
 */
public class MyPlayHistoryAdapter extends BaseAda<PlayHistoryResultDataList> {
    private DeleteItemInterface mDeleteItemInterface;
    public MyPlayHistoryAdapter(Context context) {
        super(context);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        final PlayHistoryResultDataList item= getItem(position);
        ...
        viewHolder.public_video_item_delete_iv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mDeleteItemInterface.showDeleteDialog(item.getPid(),position);
            }
        });
        PicassoUtil.loadImage(mContext,item.getResourcesCoverImg()+"?imageView2/2/w/" + (lp.width + 120), viewHolder.public_video_item_icon_iv);
        viewHolder.public_video_item_bottom_text_one_tv.setText(item.getResourcesTitle());
        viewHolder.public_video_item_bottom_text_two_tv.setText("播放次数："+item.getPlayerNumbers());
        return convertView;
    }

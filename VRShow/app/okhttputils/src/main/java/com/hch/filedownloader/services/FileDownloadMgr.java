...
class FileDownloadMgr {
    ...
    public FileDownloadMgr(Context context,final OkHttpClient client) {
 ...
 //后台服务开始下载任务， 检查是否正在下载。第一次下载任务，就新建数据库
    ...
    public synchronized void start(final String url, final String path, final int callbackProgressTimes,
                                   final int autoRetryTimes, final FileDownloadHeader header,final String urlKey) {
        ...
        else if(StringUtils.isEmpty(urlKey)){ //如果没有更新数据库，则判断urlKey是否需要刷新，需要再更新下数据库
            mHelper.updateUrlKey(model.getId(),urlKey);
        }

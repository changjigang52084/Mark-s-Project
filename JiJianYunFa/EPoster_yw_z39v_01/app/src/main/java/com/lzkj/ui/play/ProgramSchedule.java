 ...
 *@time:2015年7月3日 上午10:51:24
 *@Description: 节目调度的类
 */
public class ProgramSchedule {
	private static final LogTag TAG = LogUtils.getLogTag(ProgramSchedule.class.getSimpleName(), true);
	...
	public ProgramSchedule(List<Program> programList) {
	...
	private void initSchedule() {
		if (null == todayProgramList || todayProgramList.isEmpty()) {
			LogUtils.e(TAG, "init", "todayProgramList is null.");
			return;
		}
	...
	private void schedulePrmTask(List<Program> programList) {
		LogUtils.e(TAG, "schedulePrmTask", "schedulePrmTask.");
		if (null == programList || programList.size() == 0) {
			LogUtils.e(TAG, "schedulePrmTask", "ProgramList is null.");
			return;
		}
		for (Program program : programList) {
			if(null == program){
				LogUtils.w(TAG, "schedulePrmTask", "Program is null.");
				continue;
			}
			//调度节目开始
			...
			LogUtils.d(TAG, "schedulePrmTask", "add new program key: " + program.getKey());
		}
	}
	...
	public synchronized void updateProgramList(List<Program> newPrmList){
		...
		if (null == newPrmList || newPrmList.isEmpty()) {
			LogUtils.e(TAG, "updateProgramList", "newPrmList is null.");
			return;
		}
		if (null == todayProgramList || todayProgramList.isEmpty()) {
			LogUtils.e(TAG, "updateProgramList", "oldPrmList is null.");
			scheduleTodayProgram(newPrmList);
			return;
		}
		LogUtils.d(TAG, "updateProgramList", "newPrmList program size: " + newPrmList.size());
		...
		for (Program newProgram : newPrmList) {//遍历新节目列表
			for (Program currentProgram : tempCurrentPrmList) {//当前节目列表
				if (newProgram.getKey().equals(currentProgram.getKey())) { 
					LogUtils.d(TAG, "screenInvaildProgramList", "current program key:" + currentProgram.getKey());
	...
	private void removeInvaildProgramList(ArrayList<Program> invaildProgramList) {
		...
		for (Program delProgram : invaildProgramList) {
			LogUtils.d(TAG, "updateProgramList", "del program key: " + delProgram.getKey());
	...
	public Program getNextProgram(){
		if (currentProgramList == null || currentProgramList.isEmpty()) {
			LogUtils.i(TAG, "getNextProgram", "Current program list is null, switch default layout.");
			currentProgram = null;
	...
	private Program filterEffectiveProgram() {
		if (ListUitl.isEmpty(currentProgramList)) {
			programIndex = 0;
			return null;
		}
		//当前播放列表中有插播节目，优先播放。
		if (getIntercutProgramSize()>0) {
			...
			LogUtils.e(TAG,"filterEffectiveProgram","当前播放列表中有插播节目，优先播放");
		...
		while (ProgramParseTools.checkProgrameTimeIfOverdue(newProgram)) {
			...
			if (retry >= programListSize) {
				currentProgram = null;
				LogUtils.w(TAG, "getNextProgram", "All programs is out of play time now, switch default layout.");
				return currentProgram;
			}
			nextProgramIndex();
			newProgram = currentProgramList.get(programIndex);
			LogUtils.w(TAG, "getNextProgram", "Program key: " + newProgram.getKey() + ",todayProgramList.size:" + todayProgramList.size());
		}
	...
	private void nextProgramIndex(){
		...
		LogUtils.d(TAG, "nextProgramIndex", "programIndex：" + programIndex);
	}
	...
	private void nextIntercutProgramIndex(){
		...
		LogUtils.d(TAG, "nextIntercutProgramIndex", "intercutProgramIndex：" + intercutProgramIndex);
	}
	...
	public Program getUpcomingProgram(){
		if (null == todayProgramList || todayProgramList.isEmpty()) {
			LogUtils.w(TAG, "getUpcomingProgram", "ProgramList is empty.");
			return null;
		}
	...
	/**
	 * 延迟执行任务
	 * @param isPlayProgramTask
	 * 			true是播放任务,false是加载下一个节目任务
	 * @param delayMillis
	 * 			 延迟时间(毫秒)
	 */

...
public class LogBuilder {
	...
	/** 日志缓存大小，满则写入磁盘*/ 
	private static final int LOG_CACHE_QUEUE_SIZE = 2;
	/** 日志文件缓存大小*/ 
	...
	private void writeMessage(String text) {
		...
		File logDirectory = new File(FileStore.getInstance().getLogFolderPath());
	 ...
	 * 			日志的地址
	 */
	public static void sendUploadLogReceiver(String logPath) {
		LogUtils.d(TAG, "sendUploadLogReceiver", "logPath: " + logPath);
		Intent uploadIntent = new Intent();
		uploadIntent.setAction(Constants.UPLOAD_LOG_ACTION);
		uploadIntent.putExtra(Constants.LOG_PATH, logPath);
		EPosterApp.getApplication().sendBroadcast(uploadIntent);
	}
	...
	private void cleanCacheLogFile() {
		File logDir = new File(FileStore.getInstance().getLogFolderPath());
	...
	private boolean canWriteToDisk() {
		return new File(FileStore.getInstance().getSdcrad()).canWrite();
	}
	...
	private String getLogFileName() {
		String logFileName = LOG_PREFIX + "_" + logType + "_" + DateTimeUtil.getStringTimeToFormat("yyyy_MM_dd_HH") 
				+ "_"+ ConfigSettings.getDid() + "." + LOG_SUFFIX;
		return logFileName;
	}

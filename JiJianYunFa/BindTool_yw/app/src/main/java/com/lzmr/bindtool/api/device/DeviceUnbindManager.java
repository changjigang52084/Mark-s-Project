...
public class DeviceUnbindManager extends BaseDeviceSettingManager {

    public DeviceUnbindManager(String deviceId, SessionRequestListener listener) {
    ...
    private void unbind() {
        String unbindUrl = HttpUtil.getUnbindServer();
        if (StringUtils.isEmpty(unbindUrl)) {
            LogUtils.e(LogUtils.getStackTraceElement(), "Unbind url is null.");
            return;
        }

        MyHttpClient httpClient = new MyHttpClient();
//        if (!ConfigSettings.getBindMode()) {
            httpClient.getClient().addHeader("Authorization-Type", AuthorityCodeConstants.AUTHORITY_CODE_UNBIND);
//        }
        ...
        httpClient.post(unbindUrl, params, null, this);
    }
            ...
            if (responseContent.isSuccess()) {
                ...
                try {
                    ...
                    if (deviceControlDto != null && deviceControlDto.getSuccess()) {
                        mListener.onSuccess(BindToolApp.getApplication().getString(R.string.unbind_success));
                    } else {
                        mListener.onFailure(BindToolApp.getApplication().getString(R.string.unbind_fail));
                    }

...
public class FileUtile {
    private static final LogTag TAG_LOG = LogUtils.getLogTag(FileUtile.class.getSimpleName(), true);
    /**
     * 根目录
     */
    ...
    public String getRoot() {
        return getSDCard() + "mallposter" + File.separator;
    }
    ...
    public String getSDCard() {
        ...
        LogUtils.d(TAG_LOG, "getSDCard", "sdcardPath: " + sdcardPath);
        return sdcardPath;
    }
    ...
    public void setSDCard(String sdCard) {
        ...
        LogUtils.d(TAG_LOG, "setSDCard", "sdCard: " + sdCard);
        init();
    }
    ...
    public String getMac() {
        ...
        if (StringUtil.isNullStr(mac)) {
            mac = Util.getMacAddress().toUpperCase();
            try {
        ...
        ThreadPoolManager.get().addRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    LogUtils.d(TAG_LOG, "writePrm", "floderFile : " + floderFile.getAbsolutePath());
                    File localPrmFile = new File(floderFile, fileName + PRM_SUFFIX_NAME);
                    LogUtils.d(TAG_LOG, "writePrm", "floderFile : " + localPrmFile.getAbsolutePath());
                    if (localPrmFile.exists()) {
                        return;
                    }

                    File prmFile = new File(floderFile, fileName + TEMP_SUFFIX_NAME);
                    LogUtils.d(TAG_LOG, "writePrm", "floderFile : " + prmFile.getAbsolutePath());
                    if (!prmFile.exists()) {
                        prmFile.createNewFile();
                    }
                    String prmStr = JSON.toJSONString(prmBean);
                    LogUtils.d(TAG_LOG, "writePrm", "prmStr = " + prmStr);
    ...
    public boolean delProgramToPrmId(String prmkey) {
        LogUtils.d(TAG_LOG, "delProgramToPrmId", "prmId: " + prmkey);

        LogManager.get().insertOperationMessage("Communcation Del program prm id :" + prmkey
    ...
    public boolean delFile(File file) {
        LogManager.get().insertOperationMessage("Communcation (delFile) Del file, file path :" + file.getAbsolutePath()
                + "_" + DateTimeUtil.getStringTimeToFormat("yyyy_MM_dd_HH_mm_ss") + "\r\n");
        if (file.exists()) {
            ...
            if (null != program) {
                ...
                for (String delFileName : delFileNameList) {//遍历删除的节目列表素材
                    LogUtils.e(TAG_LOG, "delProgramToPrmId", "Check delete file: " + delFileName);
                    if (notDelFileNameList.contains(delFileName)) {//判断未删除的节目素材列表中,是否包含了要删除列表中的素材。
                        saveFileNameList.add(delFileName);
                        LogUtils.w(TAG_LOG, "delProgramToPrmId", "save file: " + delFileName);
                    }
                }
                delFileNameList.removeAll(saveFileNameList);
                for (String del : delFileNameList) {
                    LogUtils.e(TAG_LOG, "delProgramToPrmId", "Do delete file: " + del);
                }
    ...
    public static void delete(String path) {
        LogManager.get().insertOperationMessage("Communcation (delete) delete file, file path :" + path
                + "_" + DateTimeUtil.getStringTimeToFormat("yyyy_MM_dd_HH_mm_ss") + "\r\n");
    ...
    private static boolean delFileToPath(String filePath) {
        ...
        LogManager.get().insertOperationMessage("Communcation (delFileToPath) delFileToPath, file path :"
                + filePath + "_" + DateTimeUtil.getStringTimeToFormat("yyyy_MM_dd_HH_mm_ss") + "\r\n");
    ...
    public static void writePrmContent(File prmFile, String prmContent) throws IOException {
        Log.d("writePrmContent","prmFile: " + prmFile.getAbsolutePath() + " ,prmContent: " + prmContent);
    ...
    public final void writePrmListToSd(final List<Program> programList) {
        LogUtils.d(TAG_LOG, "writePrmListToSd", "writePrmListToSd ");
        ThreadPoolManager.get().addRunnable(new Runnable() {
            @Override
            public void run() {
                try {
                    LogUtils.d(TAG_LOG, "writePrmListToSd", "run ");
                    for (Program program : programList) {
                        if (null == program) {
                            continue;
                        }
                        String programJson = JSON.toJSONString(program);
                        float promTotal = (ParsePrmUitl.getProgramFileTotal(program) / 1024f / 1024f);
                        DecimalFormat decimalFormat = new DecimalFormat("#.000");
                        String programName = program.getKey() + Constant.SPLIT_PATH + decimalFormat.format(promTotal);
                        LogUtils.d(TAG_LOG, "writePrmListToSd", "programName : " + programName);
                        File file = new File(getWaitDownloadPrmFolder() + File.separator + programName);
                        LogUtils.d(TAG_LOG, "writePrmListToSd", "file programName : " + file.getAbsolutePath());
                        if (!file.exists()) {

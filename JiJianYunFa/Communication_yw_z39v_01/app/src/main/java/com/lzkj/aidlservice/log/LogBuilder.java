package com.lzkj.aidlservice.log;
...
public class LogBuilder {
	...
	private void writeMessage(String text) {
		...
		File logDirectory = new File(FileUtile.getInstance().getLogFolder());
		boolean flag = true;
		while (flag) {
			try {
...
//				File logFile = new File(logDirectory, getLogFileName(index));
	...
	private void cleanCacheLogFile() {
		File logDir = new File(FileUtile.getInstance().getLogFolder());
	...
	private boolean canWriteToDisk() {
		return new File(FileUtile.getInstance().getSDCard()).canWrite();
	}

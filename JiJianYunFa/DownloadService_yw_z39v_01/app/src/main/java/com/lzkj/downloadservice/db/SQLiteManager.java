package com.lzkj.downloadservice.db;
...
public class SQLiteManager {
    ...
    private static class SQLiteManagerInstance {
        private static final SQLiteManager sqLiteManager = new SQLiteManager(DownloadApp.getContext());
    }
...
//		contentValues.put(thread_id, threadId);
    ...
    public synchronized int queryProgresToTable(String downloadUrl, int threadId) {
        LogUtils.w(TAG, "queryProgresToTable", "sqLiteDatabase query");
        if (null == sqLiteDatabase) {
            LogUtils.w(TAG, "queryProgresToTable", "sqLiteDatabase is null");
            return 0;
        }
        Cursor cursor = sqLiteDatabase.query(download_progress_tableName, new String[]{progress},
                ...
                null, null, null);
        if (null == cursor) {
            LogUtils.w(TAG, "queryProgresToTable", "cursor is null");
            return 0;
        }
    ...
    public synchronized ArrayList<RecoveryDownloadBean> getRecovery() {
        ...
        sqLiteDatabase.beginTransaction();
        try {
            queryDownloadFileList(downloadBeans, Constant.DOWNLOAD_PRMLIST);
            queryDownloadFileList(downloadBeans, Constant.DOWNLOAD_PRMFILE);
//			queryDownloadFileList(downloadBeans, Constant.DOWNLOAD_FILE);
            queryHttpUrlForFolders(queryDownloadFolder(Constant.DOWNLOAD_FILE), Constant.DOWNLOAD_FILE, downloadBeans);
...
//			queryHttpUrlForFolders(queryDownloadFolder(Constant.DOWNLOAD_PRMLIST), Constant.DOWNLOAD_PRMLIST, downloadBeans);
    ...
    private void queryDownloadFileList(List<RecoveryDownloadBean> downloadBeans, int downloadType) {
        ...
        if (downloadPrmCursor.moveToFirst()) {
            ...
            while (downloadPrmCursor.moveToNext()) {
                ...
                addRecoveryDownloadBean(downloadBeans, downloadType, fileUrlPath, localFolder);
                LogUtils.d(TAG, "getRecovery", "fileName:" + fileUrlPath);
            }
     ...
     * @return true表示成功
     */
    public synchronized boolean insterFlow(long uploadFlow, long downloadFlow, String date) {
        boolean isSuccess = false;
        if (null != date && null != sqLiteDatabase) {
            FlowBo flowBo = queryFlowBo(date);
            if (null != flowBo) {
                if (0 != downloadFlow) {
                    downloadFlow = flowBo.downloadFlow + downloadFlow;
                    isSuccess = updateFlowTable(date, downloadFlow, 0);
                }

                if (0 != uploadFlow) {
                    uploadFlow = flowBo.uploadFlow + uploadFlow;
                    isSuccess = updateFlowTable(date, 0, uploadFlow);
                }
                return isSuccess;
            }
            ContentValues uploadFlowContentValues = new ContentValues();
            uploadFlowContentValues.put(SQLiteHelper.FLOW_DATE, date);
            if (0 != uploadFlow) {
                uploadFlowContentValues.put(SQLiteHelper.UPLOAD_FLOW,
                        String.valueOf(uploadFlow));
            }
            if (0 != downloadFlow) {
                uploadFlowContentValues.put(SQLiteHelper.DOWNLOAD_FLOW,
                        String.valueOf(downloadFlow));
            }
            try {
                sqLiteDatabase.beginTransaction();
                long result = sqLiteDatabase.insert(SQLiteHelper.TABLE_FLOW,
                        null, uploadFlowContentValues);
                if (result > 0) {
                    isSuccess = true;
                }
                sqLiteDatabase.setTransactionSuccessful();
     ...
     * @return 根据日期查询流量记录
     */
    public synchronized FlowBo queryFlowBo(String date) {
        FlowBo flowBo = null;
        if (null != sqLiteDatabase && null != date) {
            Cursor cursor = sqLiteDatabase.query(SQLiteHelper.TABLE_FLOW,
                    new String[]{SQLiteHelper.FLOW_DATE, SQLiteHelper.UPLOAD_FLOW,
                            SQLiteHelper.DOWNLOAD_FLOW, SQLiteHelper.FLOW_MONTH},
                    SQLiteHelper.FLOW_DATE + " = ?", new String[]{date}, null, null, null);
            if (null != cursor && cursor.moveToFirst()) {
                flowBo = new FlowBo();
                flowBo.date = cursor.getString(0);
                String uploadFlow = cursor.getString(1);
                String downloadFlow = cursor.getString(2);
                if (null != uploadFlow) {
                    flowBo.uploadFlow = Long.getLong(uploadFlow, 0);
                }
                if (null != downloadFlow) {
                    flowBo.downloadFlow = Long.getLong(downloadFlow, 0);
                }
                flowBo.month = cursor.getString(3);
            }
            cursor.close();
        }
        return flowBo;

...
//	}
	...
	public boolean cancelTaskToFileNames(List<String> fileNames) {
		boolean isSuccess = false;
		/***被取消的下载任务**/
		LogUtils.d(TAG, "cancelTaskToFileName", "fileNames size:" + fileNames.size());
...
//			}
	...
	private void delFile(String fileName) {
		if (!checkFileIsNotSureDel(fileName)) {
			...
			if (file.exists()) {
				file.delete();
				LogUtils.d(TAG, "delFile", "del cancel download file:" + fileName);
			}
		} else {
			LogUtils.d(TAG, "delFile", " not del fileName:" + fileName);
		}
	}

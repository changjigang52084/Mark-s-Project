...
//                TypeTokenPlaylist(event.getResult(), false);
            ...
            case InterFaceCode.DownLoadDone_load:
                ...
                handler.sendEmptyMessageDelayed(0x1234,6000);
                break;
        }
    }
...
//                    } else Showdownload();
    ...
    private void Showdownload() {
        ...
        for (PhotosEvent photosEvent : mPhotosEvent) {
            String Key = photosEvent.getKey();
            int Sm = Key.lastIndexOf(".");
            String Suffix = Key.substring(Sm);
            String PhotoPathName = photosEvent.getMd5() + Suffix;
            final String Path = CloudFrameApp.BASE + PhotoPathName;
            if (!PinJieIsPhoto(PhotoPathName))
                HttpURLUtils.Download(photosEvent.getUrl(), new File(Path), 0);
            else {
                int DownFileSizeDouble = 0;
                Long FuwuFileSizeDouble = Long.parseLong("0");
                try {
                    DownFileSizeDouble = (int) FileUtils.getInstance().getFileOrFilesSize(Path);
                    FuwuFileSizeDouble = photosEvent.getSize();
                } catch (Exception e) {
                    Log.e("TAG", "我觉得你删图片了,导致异常出现了.........");
                }
                Log.e("TAG", "本地文件大小:" + DownFileSizeDouble + "  服务器文件大小:" + FuwuFileSizeDouble);

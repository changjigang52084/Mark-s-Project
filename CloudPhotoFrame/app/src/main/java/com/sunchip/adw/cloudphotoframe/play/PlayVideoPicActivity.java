...
//        if (Ismedia)
        ...
        if (System.currentTimeMillis() - time > SLEEP_TIME) {
            Log.e("TAG", "息屏状态==================:" + mp.isPlaying() + "  System.currentTimeMillis() - SLEEP_TIME > 2000=" +
...
//        Log.e("TAG",width+"==========="+height);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(path)) {
            path = getResources().getString(R.string.on);
            SharedUtil.newInstance().setString("MusicSetting", getResources().getString(R.string.on));
        }
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (finalPath.equals(getResources().getString(R.string.off))) {
                    //关闭音乐  不播放
...
//        // 记录当前播放进度
//        outState.putBoolean("aa",MusicPlayManger.getInstance().IsPlay());
//    }
    ...
    public void PlayVideoEvent(BaseErrarEvent event) {
        switch (event.getCode()) {
            ...
            case InterFaceCode.playtimer:
                ...
                break;
            case InterFaceCode.ScreenStatu:
                if (IsVideo) {
                    //视频不用管
                    OnePlay = true;
                } else {
                ...
                Log.e("PLAY", "开屏===================OnePlay=========================" + OnePlay + " IsVideo=" + IsVideo);
                break;
        }
    }
...
//            imageView.setAdjustViewBounds(true);
    ...
    private void paustPlaylist() {
        stopThread();
        IsPause = !IsPause;
        Log.e("TAG", "是否是视频=====0========" + IsVideo);
        if (IsPause) {
            //缩短点时间呗
            PlayTimer.setStartCount(SLEEP_TIME / 2);
            Log.e("TAG", "是否还在播放=====1========" + (!mediaPlayer.isPlaying()));
            if (IsVideo)
                //如果播放的是视频才去继续播放 如果是图片的话就会异常
                if (!mediaPlayer.isPlaying()) {
                    mediaPlayer.start();
                }
        } else {
            Log.e("TAG", "是否还在播放=======2======" + (mediaPlayer.isPlaying()));
            if (mediaPlayer.isPlaying()) {
                mediaPlayer.pause();
            }
        }
    ...
    private void lift() {
        ...
        handler.sendEmptyMessageDelayed(0x123, 800);
    }
...
//        imageView.setScaleType(ImageView.ScaleType.FIT_XY);
        if (isplay) {
            Glide.with(this).load(materialPath).into(imageView);
            //毛玻璃特效
            try {
                Bitmap bitmap = BitmapUtils.getInstance().thumbleImages(materialPath);
                int scaleRatio = 40;
                int blurRadius = 8;
...
//                    Log.e("TAG","需要显示的数据是:"+mPhotosEvent.get(i).getCaption());
                    ...
                    if (FileUtils.getInstance().getFileOrFilesSize(materialPath) == mPhotosEvent.get(i).getSize()) {
                        if (MediaFileUtils.isImageFileType(materialPath)) {
                            if (OnePlay) MusicPlayManger.getInstance().play();
                            else
                    ...
                    Log.e("TAG", "md5不匹配=====说明数据列表没有解析完成出现异常,那么就下一张===");
//                    PlayTimer.setStartCount(SLEEP_TIME / 2);
                }
            }
    ...
    private void initData() {
        ...
        if (SharedUtil.newInstance().getInt("shuffle") == 1) {
            ...
            index++;
        handler.removeMessages(0x123);
        handler.sendEmptyMessageDelayed(0x123, 600);
    }
